...
def show_Game(ort,klass,kön):
    check_date()
    print(ort,klass,kön)
    sql = "select ort, klass, antal, matchid, kön, datum from match where ort = %s AND klass = %s AND kön = %s AND antal > 0 AND skapare != %s"
...
def show_all_match(ort):
    ...
    sql = "select ort, klass, antal, matchid, kön, datum from match where ort = %s AND antal > 0 AND skapare != %s"
...
def show_all_ranks(ort, klass):
    check_date()
    sql = "select ort, klass, antal, matchid, kön, datum from match where ort = %s and klass = %s AND antal > 0 AND skapare != %s"
...
def show_all_players(ort, kön):
    check_date()
    print(kön)
    sql = "select ort, klass, antal, matchid, kön, datum from match where ort = %s and kön = %s AND antal > 0 AND skapare != %s"
